...
public class LockingValuesTest extends TestBaseCluster {
    ...
    void testSpecificLockAndRestrictedUnlockCreateOnMasterValidateOnBackup() throws ExecutionException, InterruptedException {
        ...
        LockStatus lockRes = client.setMode(FastCacheAsyncSmartClient.Mode.BACKUP).lockObject(lockKey1, LockType.WRITE_LOCK, ownerId, Duration.ofSeconds(60)).get();
        Assertions.assertEquals(LockStatus.OK, lockRes);

        // 2. Attempt unlock by intruder (Should be rejected)
        LockStatus failedUnlock = client.setMode(FastCacheAsyncSmartClient.Mode.BACKUP).unlockObject(lockKey1, intruderId).get();
        ...
        LockStatus successUnlock = client.setMode(FastCacheAsyncSmartClient.Mode.BACKUP).unlockObject(lockKey1, ownerId).get();
    ...
    void testSpecificLockAndRestrictedUnlockCreateOnBackupValidateOnMaster() throws ExecutionException, InterruptedException {
        ...
        // 1. Lock by specific owner
        LockStatus lockRes = client.setMode(FastCacheAsyncSmartClient.Mode.MASTER).lockObject(lockKey1, LockType.WRITE_LOCK, ownerId, Duration.ofSeconds(60)).get();
        Assertions.assertEquals(LockStatus.OK, lockRes);

        // 2. Attempt unlock by intruder (Should be rejected)
        LockStatus failedUnlock = client.setMode(FastCacheAsyncSmartClient.Mode.MASTER).unlockObject(lockKey1, intruderId).get();
        Assertions.assertEquals(LockStatus.CANT_UNLOCK,
                                failedUnlock,
                                "Intruder should not be able to unlock owner's lock");

        // 3. Attempt unlock by owner (Should succeed)
        LockStatus successUnlock = client.setMode(FastCacheAsyncSmartClient.Mode.MASTER).unlockObject(lockKey1, ownerId).get();
        Assertions.assertEquals(LockStatus.OK, successUnlock);
    }
    ...
    void testLockingConflictCreateOnMasterValidateOnBackup() throws ExecutionException, InterruptedException {
        ...
        client.setMode(FastCacheAsyncSmartClient.Mode.BACKUP).lockObject(lockKey1, LockType.WRITE_LOCK, 1, Duration.ofSeconds(60)).get();

        // 2. Client B tries to lock the same object (Should fail)
        LockStatus conflictRes = client.setMode(FastCacheAsyncSmartClient.Mode.BACKUP).lockObject(lockKey1, LockType.WRITE_LOCK, 2, Duration.ofSeconds(60)).get();
    ...
    void testLockingConflictCreateOnBackupValidateOnMaster() throws ExecutionException, InterruptedException {
        ...
        client.setMode(FastCacheAsyncSmartClient.Mode.MASTER).lockObject(lockKey1, LockType.WRITE_LOCK, 1, Duration.ofSeconds(60)).get();

        // 2. Client B tries to lock the same object (Should fail)
        LockStatus conflictRes = client.setMode(FastCacheAsyncSmartClient.Mode.MASTER).lockObject(lockKey1, LockType.WRITE_LOCK, 2, Duration.ofSeconds(60)).get();
        Assertions.assertEquals(LockStatus.CANT_LOCK, conflictRes, "Should not allow double locking");
    }
    ...
    void testUnanimousLockBlocksSpecificLockCreateOnMasterValidateOnBackup() throws ExecutionException, InterruptedException {
        ...
        client.setMode(FastCacheAsyncSmartClient.Mode.BACKUP).lockObject(lockKey1, LockType.WRITE_LOCK, 0, Duration.ofSeconds(60)).get();

        // 2. Client 1 tries to lock specifically (Should fail because it's already locked)
        LockStatus conflictRes = client.setMode(FastCacheAsyncSmartClient.Mode.BACKUP).lockObject(lockKey1, LockType.WRITE_LOCK, 1, Duration.ofSeconds(60)).get();
    ...
    void testUnanimousLockBlocksSpecificLockCreateOnBackupValidateOnMaster() throws ExecutionException, InterruptedException {
        ...
        client.setMode(FastCacheAsyncSmartClient.Mode.MASTER).lockObject(lockKey1, LockType.WRITE_LOCK, 0, Duration.ofSeconds(60)).get();

        // 2. Client 1 tries to lock specifically (Should fail because it's already locked)
        LockStatus conflictRes = client.setMode(FastCacheAsyncSmartClient.Mode.MASTER).lockObject(lockKey1, LockType.WRITE_LOCK, 1, Duration.ofSeconds(60)).get();
        Assertions.assertEquals(LockStatus.CANT_LOCK, conflictRes);
    }
    ...
    void testUnlockOnExpiredObjectCreateOnMasterValidateOnBackup() throws ExecutionException, InterruptedException {
        ...
        // Create should be done on master
        String lockKey1 = lockKey;
        client.setMode(FastCacheAsyncSmartClient.Mode.MASTER).createKeyValue(lockKey1, "initial_data".getBytes(StandardCharsets.UTF_8)).get();
        // Allow cache to replicate data inside cluster
        Thread.sleep(500);

        // Lock on backup
        LockStatus lockStatus = client.setMode(FastCacheAsyncSmartClient.Mode.BACKUP).lockObject(lockKey1, LockType.WRITE_LOCK, 555, Duration.ofSeconds(1)).get();
        ...
        LockStatus res = client.setMode(FastCacheAsyncSmartClient.Mode.BACKUP).unlockObject(lockKey1, 999).get();
    ...
    void testGlobalLockBlocksDataAccessCreateOnMasterValidateOnBackup() throws ExecutionException, InterruptedException {
        ...
        // Create should be done on master
        client.setMode(FastCacheAsyncSmartClient.Mode.MASTER).createKeyValue(key, "sensitive_info".getBytes()).get();
        // Allow cache to replicate data inside cluster
        Thread.sleep(500);

        // Lock on backup
        client.setMode(FastCacheAsyncSmartClient.Mode.BACKUP).lockObject(key, LockType.GLOBAL, ownerId, Duration.ofSeconds(60)).get();

        // 2. Owner should be able to read (requires clientId in GetRequest)
        // Assuming your getValue is updated to pass clientId
        byte[] data = client.setMode(FastCacheAsyncSmartClient.Mode.BACKUP).getValue(key, ownerId).get();
        Assertions.assertNotNull(data);

        // 3. Intruder tries to read (Should fail)
        try {
            client.setMode(FastCacheAsyncSmartClient.Mode.BACKUP).getValue(key, intruderId).get();
    ...
    void testGlobalLockBlocksDataAccessCreateOnBackupValidateOnMaster() throws ExecutionException, InterruptedException {
        ...
        // 1. Create and then Lock Globally
        // Create should be done on backup
        client.setMode(FastCacheAsyncSmartClient.Mode.BACKUP).createKeyValue(key, "sensitive_info".getBytes()).get();
        // Allow cache to replicate data inside cluster
        Thread.sleep(500);

        // Lock on master
        client.setMode(FastCacheAsyncSmartClient.Mode.MASTER).lockObject(key, LockType.GLOBAL, ownerId, Duration.ofSeconds(60)).get();

        // 2. Owner should be able to read (requires clientId in GetRequest)
        // Assuming your getValue is updated to pass clientId
        byte[] data = client.setMode(FastCacheAsyncSmartClient.Mode.MASTER).getValue(key, ownerId).get();
        Assertions.assertNotNull(data);

        // 3. Intruder tries to read (Should fail)
        try {
            client.setMode(FastCacheAsyncSmartClient.Mode.MASTER).getValue(key, intruderId).get();
            Assertions.fail("Intruder should have been blocked by GLOBAL lock");
        } catch (ExecutionException e) {
            StatusRuntimeException cause = (StatusRuntimeException) e.getCause();
            Assertions.assertEquals(Status.Code.PERMISSION_DENIED, cause.getStatus().getCode());
