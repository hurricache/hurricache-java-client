...
public class QueueOperationsReplicationTest extends TestBaseCluster {
    ...
    void testQueueLifecycleCreateOnMasterGetOnBackup() throws ExecutionException, InterruptedException {
        ...
        boolean removed = client.setMode(Mode.BACKUP).removeHead(qKey).get();
        Assertions.assertTrue(removed);

        // 7. Verify Queue is now empty or key doesn't exist
        Thread.sleep(500);
        byte[] emptyB = client.setMode(Mode.BACKUP).getHead(qKey, keyHint).get();
        byte[] emptyM = client.setMode(Mode.MASTER).getHead(qKey, keyHint).get();
    ...
    void testQueueOrderPersistenceCMGB() throws ExecutionException, InterruptedException {
        ...
        Assertions.assertEquals("1", new String(client.setMode(Mode.MASTER).getAndRemoveFront(qKey).get()));
        Thread.sleep(500);
        Assertions.assertEquals("2", new String(client.setMode(Mode.BACKUP).getAndRemoveFront(qKey).get()));
        Thread.sleep(500);
        Assertions.assertEquals("3", new String(client.setMode(Mode.MASTER).getAndRemoveFront(qKey).get()));
    }
    ...
    void testQueueOrderPersistenceCBGM() throws ExecutionException, InterruptedException {
        ...
        Thread.sleep(500);
        // FIFO verification: 1 -> 2 -> 3
        Assertions.assertEquals("1", new String(client.setMode(Mode.BACKUP).getAndRemoveFront(qKey).get()));
        Thread.sleep(500);
        Assertions.assertEquals("2", new String(client.setMode(Mode.MASTER).getAndRemoveFront(qKey).get()));
        Thread.sleep(500);
        Assertions.assertEquals("3", new String(client.setMode(Mode.BACKUP).getAndRemoveFront(qKey).get()));
    }
