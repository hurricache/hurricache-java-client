...
public class QueueOperationsTest extends TestBaseCluster {
    ...
    void testQueueLifecycleCreateOnMasterValidateOnBackup() throws ExecutionException, InterruptedException {
        ...
        KeyHint keyHint = client.setMode(Mode.MASTER).createQueue(qKey, List.of(first)).get();
        ...
        boolean added = client.setMode(Mode.BACKUP).addElementToTail(qKey, keyHint, List.of(second)).get();
    ...
    void testQueueLifecycleCreateOnBackupValidateOnMaster() throws ExecutionException, InterruptedException {
        ...
        KeyHint keyHint = client.setMode(Mode.BACKUP).createQueue(qKey, List.of(first)).get();
        Assertions.assertNotNull(keyHint);
        // Allow cache to replicate data inside cluster
        Thread.sleep(500);

        // 2. addElementToTail on master
        boolean added = client.setMode(Mode.MASTER).addElementToTail(qKey, keyHint, List.of(second)).get();
    ...
    void testQueueOrderPersistenceCreateOnMasterValidateOnBackup() throws ExecutionException, InterruptedException {
        ...
        byte[] first = createLargePayload(VALUE_SIZE);
        byte[] second = createLargePayload(VALUE_SIZE);
        client.setMode(Mode.BACKUP).addElementToTail(qKey, keyHint, Arrays.asList(first,
                                                                                  second)).get();

        // FIFO verification: 1 -> 2 -> 3
        Assertions.assertArrayEquals(initial, client.setMode(Mode.BACKUP).getAndRemoveFront(qKey, keyHint).get());
        Assertions.assertArrayEquals(first, client.setMode(Mode.BACKUP).getAndRemoveFront(qKey, keyHint).get());
        Assertions.assertArrayEquals(second, client.setMode(Mode.BACKUP).getAndRemoveFront(qKey, keyHint).get());
    }
    ...
    void testQueueOrderPersistenceCreateOnBackupValidateOnMaster() throws ExecutionException, InterruptedException {
        ...
        byte[] first = createLargePayload(VALUE_SIZE);
        byte[] second = createLargePayload(VALUE_SIZE);
        client.setMode(Mode.MASTER).addElementToTail(qKey, keyHint, Arrays.asList(first,
                                                                                  second)).get();

        // FIFO verification: 1 -> 2 -> 3
        Assertions.assertArrayEquals(initial, client.setMode(Mode.MASTER).getAndRemoveFront(qKey, keyHint).get());
        Assertions.assertArrayEquals(first, client.setMode(Mode.MASTER).getAndRemoveFront(qKey, keyHint).get());
        Assertions.assertArrayEquals(second, client.setMode(Mode.MASTER).getAndRemoveFront(qKey, keyHint).get());
    }
