...
public class FastCacheAsyncSmartClient implements FastCacheClientInterface {
        ...
        future.orTimeout(defaultTimeout.toMillis(), TimeUnit.MILLISECONDS).thenAccept(peerRoutingList -> {
            try {
                ...
                oldTargets.stream().filter(t -> !newTargets.contains(t)).forEach(target -> {
                    FastCacheClientInterface oldClient = newRoutingTableTarget.remove(target);
                    if (oldClient != null) {
                        oldClient.shutdown();
                    }
                });
    ...
    private <T> CompletableFuture<T> execute(KeyHint hint,
        ...
        FastCacheClientInterface master = getRoute(routingInfo, shard, MASTER);
        FastCacheClientInterface backup = getRoute(routingInfo, shard, BACKUP);
        ...
        return switch (effectiveMode) {
            ...
            case LB_SMART -> {
                ...
                FastCacheClientInterface primary = tryMasterFirst
                                                   ? master
                                                   : backup;
                FastCacheClientInterface secondary = tryMasterFirst
                                                     ? backup
                                                     : master;
    ...
    private <T> BiFunction<T, Throwable, CompletableFuture<T>> fallbackGuard(int shard,
                                                                             ...
                                                                             Function<FastCacheClientInterface, CompletableFuture<T>> action,
                                                                             FastCacheClientInterface currentEndpoint,
                                                                             FastCacheClientInterface fallbackEndpoint) {
        ...
        return (result, ex) -> {
            ...
            if (isReroute(ex)) {
                ...
                FastCacheClientInterface directClient = getRoute(routingInfo, targetRoute);
    ...
    public CompletableFuture<Boolean> removeElementAtPosition(byte[] key,
                                                              ...
                                                              int clientId,
                                                              Duration timeout) {
        return execute(hint, c -> c.removeElementAtPosition(key, hint, pos, clientId, timeout));
    }
